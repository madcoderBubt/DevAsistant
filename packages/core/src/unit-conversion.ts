/**
 * Configuration options for unit conversions.
 */
interface ConversionOptions {
  rootFontSize?: number;    // Default: 16 (or calculated from html)
  elementFontSize?: number; // Default: 16 (for em conversions)
 ...
 */
export const pxToRem = (px: number, rootFontSize: number = getRootFontSize()): number => {
 ...
 * @param rootFontSize The base font size (default: 16px or calculated).
 */
export const remToPx = (rem: number, rootFontSize: number = getRootFontSize()): number => {
 ...
 */
export const pxToEm = (px: number, contextFontSize: number = 16): number => {
 ...
 * @param contextFontSize The font size of the element/parent (default: 16px).
 */
export const emToPx = (em: number, contextFontSize: number = 16): number => {
 ...
 */
export const pxToVw = (px: number, width: number = (typeof window !== 'undefined' ? window.innerWidth : 1920)): number => {
  return (px / width) * 100;
};
 ...
 * @param width The viewport width (default: window.innerWidth).
 */
export const vwToPx = (vw: number, width: number = (typeof window !== 'undefined' ? window.innerWidth : 1920)): number => {
  return (vw / 100) * width;
};
 ...
 */
export const pxToVh = (px: number, height: number = (typeof window !== 'undefined' ? window.innerHeight : 1080)): number => {
 ...
 * @param height The viewport height (default: window.innerHeight).
 */
export const vhToPx = (vh: number, height: number = (typeof window !== 'undefined' ? window.innerHeight : 1080)): number => {
 ...
 */
export const pxToPercent = (px: number, containerSize: number): number => {
 ...
 * @param containerSize The size of the parent container in pixels.
 */
export const percentToPx = (percent: number, containerSize: number): number => {
